                ...
                if (!roughDic.ContainsKey(key))
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        foreach (DataRow r in dt.Rows)
                        {
                            ...
                            foreach (DataRow cfgR in cfgDt.Rows)
                            {
                                if (!roughDic.ContainsKey(string.Concat(gs_guid, xs_guid, cfgR.Field<string>("vegname"))))
                                {
                                    roughDic.Add(string.Concat(gs_guid, xs_guid, cfgR.Field<string>("vegname")), Convert.ToDecimal(cfgR["roughweight"]));
                                    if (cfgR.Field<string>("vegname").Equals(vegname))
                                    {
