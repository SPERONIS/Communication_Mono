                ...
                using (Socket publisher = Socket.Create(SocketType.Pub, cfg.BindingType))
                {
                    try
                    {
                        ...
                        while (isWork)
                        {
                            try
                            {
                                if (watch2.ElapsedMilliseconds > 10000)
                                {
                                    publisher.Send(Global.DC_CTRL_HB, BitConverter.GetBytes(DateTime.Now.Ticks));
                                ...
                                receiver.TryReceive(300, out envelope, out data);
                                ...
                                publisher.TrySend(300, envelope, data);
                                length += 1;
                                ...
                                if (watch.ElapsedMilliseconds > 20000)
                                {
                                    ...
                                    publisher.TrySend(300, Global.DC_CTRL_HB, BitConverter.GetBytes(DateTime.Now.Ticks));
                                    DCLogger.LogInfo("Server's HeartBeat sended:{0}", DateTime.Now);
                                }
                            }
                            catch (Exception e)
